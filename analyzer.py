...
class SiteAnalyzer:
    ...
    async def analyze_site(self, url, proxy=None):
        ...
        for src_name, src_text in match_sources.items():
            if not src_text:
                continue
            found = self._match_signatures(src_text, self.GATEWAY_SIGNATURES)
            for gw in found:
                all_found.add(gw)
        ...
        # Fallback: Detect Generic Gateways via keywords
        if not gateways:
    ...
    def _redact_secret(self, s: str) -> str:
        # Don't output full secrets; provide enough proof for reporting.
        s = s or ""
        if len(s) <= 10:
            return "***REDACTED***"
        return f"{s[:6]}…{s[-4:]} (redacted)"
